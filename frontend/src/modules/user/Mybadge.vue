  ...
  <div>
        ...
        <b-row v-for="(badge, index) in badgelist" :key="index" class="row" >
          ...
          <div class="cerBody" v-bind:class="{zoomin: resized}">
            <p class="name">
              This certificate of
              <br>
              <span class="cern">{{badge.badgeID.certificateName}}</span>
            </p>
            ...
            <p>Approved by {{ badge.badgeID.approvedBy }}</p>
    ...
    <div class="text-left pcont">
     <div class="pndngb" v-for="(pb, i) in pending" :key="i">
       <p class="pl p2">{{pb.badgeID.badgename}}</p>
       <p class="pl p3">{{pb.badgeID.organization}}</p>
       <p class="pl p3 d8">{{pb.badgeID.date.month+" "+pb.badgeID.date.day+", "+pb.badgeID.date.year}}</p>
       <hr class="phr">
     </div>
    </div>
    </div>
  ...
  methods: {
    searchBadge() {
      ...
      axios
        ...
        .then(res => {
          this.availing = false;
          this.reset();
          this.availedbadge.badgename = res.data.badgename;
    ...
    getData() {
      ...
      .then(res => {
        ...
        console.log(res.data.pendingbadges);
